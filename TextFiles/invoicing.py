...
def parse_invoice_number(invoice_number: str) -> tuple[int, int]:
    ...
    """
    inv_info = invoice_number.split('-')
    tup_info = tuple(int(s) for s in inv_info)
    return tup_info
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    year = get_year()
    inv_now = parse_invoice_number(invoice_number)
    if inv_now[0] == year:
        new_inv_no = f"{year}-{(inv_now[1] + 1):04d}"
    else:
        new_inv_no = f"{year}-0001"
    return new_inv_no
